...
##### the algorithm
its = c( 15, 8 )
jj = jointSmoothMatrixReconstruction( x, 4, params, positivity=F,
 gamma = 1e-6, sparsenessQuantile=0.25, iterations=its[1], subIterations = its[2],
 smoothingMatrix = slist, verbose=TRUE )
mm=makeImage( baseImage1, abs(jj$v[[2]][,1])*1e5 ) %>% iMath("Normalize")
plot( baseImage1, mm, doCropping=F, window.overlay=c(0.05,1) )
mm=makeImage( baseImage2, abs(jj$v[[1]][,1]) ) %>% iMath("Normalize")
plot( simpleImage, mm, doCropping=T, window.overlay=c(0.1,1) )
mm=makeImage( baseImage3, abs(jj$v[[3]][,1]) ) %>% iMath("Normalize")
plot( simpleImage, mm, doCropping=T, window.overlay=c(0.1,1) )
mm=makeImage( baseImage3, abs(jj$v[[3]][,2]) ) %>% iMath("Normalize")
plot( simpleImage, mm, doCropping=T, window.overlay=c(0.1,1) )
mm=makeImage( baseImage3, abs(jj$v[[3]][,3]) ) %>% iMath("Normalize")
plot( simpleImage, mm, doCropping=T, window.overlay=c(0.1,1) )
mm=makeImage( baseImage3, abs(jj$v[[3]][,4]) ) %>% iMath("Normalize")
plot( simpleImage, mm, doCropping=T, window.overlay=c(0.1,1) )
p1 = mat2 %*% jj$v[[1]]
p2 = mat1 %*% jj$v[[2]]
p3 = mat3 %*% jj$v[[3]]
...
p5 = mat3 %*% jj$v[[5]]
p6 = mat2 %*% jj$v[[6]]
print( summary( lm( p2 ~ ( education+age ) * apoe ) ) )
######################
```
