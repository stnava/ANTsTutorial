...
demog = read.csv( paste( bd2, "data/ptbp_summary_demographics.csv", sep='' ) )
if ( ! exists("demog") )
  {
...
cbftem = paste(bd2,"./data/template/PTBP_CBF.nii.gz",sep='')
...
exts=c( "mprage_t1.nii.gz" ,
        ...
        "AAL.nii.gz" )
pre=paste( bd2,"data/Subjects/",id,"/",dt,"/*",sep='')
...
if ( !exists("ilist") )
  ilist=imageFileNames2ImageList(
    Sys.glob( paste(bd2,"data/JLF/ilist*.nii.gz",sep='')) )
if ( !exists("seglist") )
  seglist=imageFileNames2ImageList(
    Sys.glob( paste(bd2,"data/JLF/slist*.nii.gz",sep='')) )
mk=getMask(t1tem)
...
if ( ! exists("jlf") ) {
tf=paste(bd2,"/data/JLF_out/jlfout_",sep='')
