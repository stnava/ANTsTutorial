...
## Merge this images to establish a single mask
...
plot( thkmask, mrvR2, axis=3, nslices=16,
   window.overlay=c(thr2, max(mrvR2) ) )
```
...
## Investigate the final model (single regression)

```{r mrvmdl}
mrvdf=data.frame( mydf, mrvpred=rfpred )
mrvmdl=lm( VIQ ~ . , data=mrvdf )
knitr::kable( coefficients( mrvmdl ) )
```
