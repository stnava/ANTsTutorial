...
inmatrix=list( scale(temp,scale=FALSE) ,
  scale( data.matrix(sdemog[traing,]), scale=FALSE) )
if ( ! exists("sccan") )
...
```{r thkcca4,eval=FALSE,echo=FALSE}
...
```{r thkcca3,eval=FALSE,echo=FALSE}
...
temp3=thkmat[haveAllMerge,]
temp=famat[haveAllMerge,][traing,]
# for ( i in 1:nrow(temp2) )
#    temp2[i,]=temp2[i,]-mean(temp2[i,])
ranker=40
# tempw2=randcca( temp3, temp, ranker, 1 )
# tempw=randcca( temp, temp3, ranker, 1 )
# tempw2p=randcca( temp3, temp[ sample(1:nrow(temp)), ], ranker, 1 )
tempw=randwhiten( temp, ranker, 1 )
tempw2=randwhiten( temp2, ranker, 1 )
inmatrix=list( scale(tempw) , scale( tempw2 ) )
if ( ! exists("fcsccan") )
  fcsccan=sparseDecom2(inmatrix, c(famask, thkmask),
     sparseness = c( 0.05, -0.05 ), verbose=1,
...
mdl=lm( AgeAtScan ~ ., data=ccadf[traing,] )
