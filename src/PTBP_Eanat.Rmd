...
## Thickness Viz
```{r thkv}
plot( thkmask, ethk$eseg , dorot=1 )
```


## CBF Viz
```{r cbfv}
plot( thkmask, ecbf$eseg , dorot=1 )
```


## FA Viz
```{r fav}
plot( famask, efa$eseg , dorot=1 )
```
...
```{r thkcca,eval=TRUE}
sdemog=antsrimpute( demog[haveAllMerge,c(3,7,8,13)] )
...
if ( ! exists("sccan") )
  sccan=sparseDecom2(inmatrix, c(thkmask, NA),
     sparseness = c( 0.05, 0.3 ),
     nvecs = 3, its = 2, cthresh = c(5000, 0),
...
mdl=randomForest( AgeAtScan ~ X1+X2+X3, data=ccadf[traing,], ntrees=5500 )
...
inmatrix=list( temp ,
  data.matrix(sdemog[traing,])  )
if ( ! exists("fsccan") )
  fsccan=sparseDecom2(inmatrix, c(famask, NA),
     sparseness = c( 0.05, 0.3 ),
     nvecs = 3, its = 2, cthresh = c(5000, 0),
     perms = 0, smooth = 0.5, mycoption = 1 )
```
...
```{r facca4,eval=TRUE}
ccaeseg=eigSeg( famask, fsccan$eig1, FALSE  )
...
```{r facca2,eval=TRUE}
rownames( fsccan$eig2 )=colnames( sdemog )
knitr::kable( fsccan$eig2 )
```

## Quick CCA-based prediction

```{r facca3,eval=TRUE}
fsccanmat=imageListToMatrix( fsccan$eig1, famask)
fsccanmat=fsccanmat/rowSums(fsccanmat)
fsccanproj = famat[haveAllMerge,] %*% t( fsccanmat )
ccadf=data.frame( sdemog, fsccanproj,
  gender=demog$Sex[haveAllMerge] )
lmdl=lm( AgeAtScan ~ .,
  data=ccadf[traing,] )
...
fcsccanmat=fsccanmat/rowSums(fcsccanmat)
